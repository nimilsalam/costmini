 ...
 *
 * Strategy: Uses Truemeds' search API endpoint which returns JSON.
...
export class TruemedsScraper extends DrugScraper {
  ...
  private async scrapeSearchPage(query: string): Promise<ScraperResult> {
    ...
    $("[class*='ProductCard'], [class*='medicine-card'], [class*='product-item']").each(
      (_, el) => {
        try {
          const $el = $(el);
          const name = $el
            .find("[class*='productName'], [class*='medicine-name'], h2, h3")
            .first()
            .text()
            .trim();
          const priceText = $el
            .find("[class*='sellingPrice'], [class*='truemed-price'], [class*='our-price']")
            .first()
            .text();
          const mrpText = $el
  ...
  async getDrugDetails(url: string): Promise<ScrapedDrug | null> {
    try {
      ...
      const composition = $(
        ...
        .trim();
      const priceText = $(
        "[class*='sellingPrice'], [class*='truemed-price'], [class*='our-price']"
      )
        .first()
        .text();
      const mrpText = $(
        ...
        .first()
        .text();
      const packSize = $("[class*='pack-size'], [class*='quantity']")
        .text()
        .trim();
      const rxRequired =
