 ...
 *
 * Strategy: Uses Apollo's search API endpoint which returns JSON.
...
export class ApolloScraper extends DrugScraper {
  ...
  private async scrapeSearchPage(query: string): Promise<ScraperResult> {
    ...
    $("[class*='ProductCard'], [class*='product-item'], [class*='medicine-card']").each(
      (_, el) => {
        try {
          const $el = $(el);
          const name = $el
  ...
  async getDrugDetails(url: string): Promise<ScrapedDrug | null> {
    try {
      ...
      const composition = $(
        ...
        .trim();
      const priceText = $(
        "[class*='selling-price'], [class*='finalPrice'], [class*='our-price']"
      )
        .first()
        .text();
      const mrpText = $(
