...
import { Search, TestTube, MapPin, Home, Share2 } from "lucide-react";
import { sampleDiagnostics, diagnosticCategories } from "@/lib/sample-data";
...
export default function DiagnosticsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (homeOnly) {
      tests = tests.filter((t) => t.homeCollection);
    }
    return tests;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">
          Lab Tests & Diagnostic Prices
        </h1>
        <p className="text-gray-500">
          Compare blood tests, MRI, CT scans and more across labs. Book at the
          best price.
        </p>
      </div>
      ...
      <div className="space-y-6">
        {filtered.map((test) => {
          const cheapest = Math.min(...test.prices.map((p) => p.sellingPrice));
          const mostExpensive = Math.max(...test.prices.map((p) => p.mrp));
