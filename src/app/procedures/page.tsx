...
import { Search, Stethoscope, MapPin, Share2 } from "lucide-react";
import { sampleProcedures, procedureCategories } from "@/lib/sample-data";
...
export default function ProceduresPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (category !== "All") {
      procs = procs.filter((p) => p.category === category);
    }
    return procs;
  }, [query, category]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500">
          Compare surgery costs across hospitals. Find affordable, accredited
          options.
        </p>
      </div>
      ...
      <div className="space-y-6">
        {filtered.map((proc) => {
          const minPrice = Math.min(...proc.prices.map((p) => p.minPrice));
          const maxPrice = Math.max(...proc.prices.map((p) => p.maxPrice));
          const shareText = `🏥 ${proc.name} — From ${formatPrice(minPrice)} on CostMini!\nCompare hospital prices: costmini.in/procedures`;
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className="text-lg font-bold text-gray-900">
                          {formatPrice(hp.avgPrice)}
                        </div>
                        <div className="text-xs text-gray-400">
                          {formatPrice(hp.minPrice)} – {formatPrice(hp.maxPrice)}
                        </div>
                      </div>
                    </div>
                  ))}
